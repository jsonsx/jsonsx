...
export function compileClient(raw, opts) {
  ...
  // Classify $defs into state, bind, on
  ...
  for (const [key, def] of Object.entries(defs)) {
    ...
    if (def.$prototype === "LocalStorage" || def.$prototype === "SessionStorage") {
      stateEntries.push([key, def.default ?? null]);
      continue;
    }
  ...
  const moduleContent = emitClientModule(stateEntries, bindEntries, onEntries, reactivitySrc);
...
function emitClientModule(stateEntries, bindEntries, onEntries, reactivitySrc) {
  const lines = [];
  ...
  if (onEntries.length > 0) {
    ...
    for (const [key, def] of onEntries) {
      const paramList = def.args?.join(", ") ?? "$defs";
      // Wrap so the first arg is always $defs
      lines.push(`  ${key}: (e) => { const fn = (${paramList}) => { ${def.body} }; fn($defs, e); },`);
    }
    lines.push("};");
  } else {
    lines.push("const on = {};");
