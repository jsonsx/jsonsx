...
Generates a standalone Hono app for a single document's server entries. Used when no `build.provider` is set:
...
When `build.provider` is set in `project.json`, the site build collects all `timing: "server"` entries across every component and page, deduplicates by export name, and emits a single `_worker.js` at the project root via `compileSiteServer()`. Per-route `_server.js` files are not generated in this mode.
...
```js
compileSiteServer(entries, { provider, baseUrl })
```

| Parameter | Type             | Default          | Description                                       |
| --------- | ---------------- | ---------------- | ------------------------------------------------- |
| `entries`  | `Array<{exportName, src}>` | —     | Pre-collected server entries from all components   |
| `provider` | `string \| null` | `null`          | Deployment provider; adds platform-specific output |
...
Example generated `_worker.js` for `provider: "cloudflare"`:
...
> **Status: Implemented.** `compile-server.js` exports `compileServer` (per-route) and `compileSiteServer` (site-wide). `site-build.js` orchestrates entry collection and `_worker.js` generation when `build.provider` is set.
...
| Image optimization                   | Format conversion, responsive sizes, lazy loading              | **Implemented** via `image-transform.js`                                      |
| Site-wide server bundling            | `build.provider` collects all server entries into `_worker.js` | **Implemented** — Cloudflare provider with asset fallback                     |
