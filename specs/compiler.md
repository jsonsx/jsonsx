...
When `build.provider` is set in `project.json`, the site build collects all `timing: "server"` entries across every component and page, deduplicates by export name, and emits a single `dist/worker.js` via `compileSiteServer()`. Per-route `_server.js` files are not generated in this mode.
...
Example generated `dist/worker.js` for `provider: "cloudflare"`:
...
> **Status: Implemented.** `compile-server.js` exports `compileServer` (per-route) and `compileSiteServer` (site-wide). `site-build.js` orchestrates entry collection and `dist/worker.js` generation when `build.provider` is set. Server source files are copied into `dist/components/` so the worker's relative imports resolve.
...
| Site-wide server bundling            | `build.provider` collects all server entries into `dist/worker.js` | **Implemented** — Cloudflare provider with asset fallback                     |
